// api/index.js
/**
 * Servidor Express da API Couple Sync.
 * Rotas:
 *  - Autenticação: POST /api/register, POST /api/login
 *  - Fotos (auth): GET /api/fotos, POST /api/upload, PUT /api/fotos/:id, DELETE /api/fotos/:id
 * Dependências externas: PostgreSQL (pg), Cloudinary (upload), JWT, Bcrypt.
 */
...
app.put("/api/fotos/:id", authMiddleware, async (req, res) => {
  try {
    ...
    if (!description || !photoDate) {
      return res
        .status(400)
        .json({ error: "Descrição e data são obrigatórias." });
    }
